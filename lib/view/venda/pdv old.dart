...
class _PDVPageState extends State<PDVPage> with SingleTickerProviderStateMixin {
  ...
  Map<int, int> _carrinho = {}; // produtoIdPublic -> quantidade
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
          ? Center(child: CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(Color(0xFFF44336))))
          : Column(
              children: [
                // Tabs
                Container(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Text(
              label,
              style: TextStyle(color: selected ? Color(0xFFF44336) : Colors.black87, fontWeight: FontWeight.bold, fontSize: 16),
            ),
            if (showBadge)
              Container(
                margin: EdgeInsets.only(left: 6),
                padding: EdgeInsets.all(6),
                decoration: BoxDecoration(color: Color(0xFFF44336), shape: BoxShape.circle),
                child: Text(
                  badgeCount.toString(),
              ...
              children: [
                Expanded(
                  child: TextField(
                    ...
                    decoration: InputDecoration(hintText: 'Buscar produto', border: InputBorder.none, contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 14)),
                  ),
                ),
                Icon(Icons.search, color: Color(0xFFF44336)),
                SizedBox(width: 8),
              ],
            ),
          ),
                ...
                GestureDetector(
                  ...
                  child: Column(
                    children: [
                      Text(
                        'Todos',
                        style: TextStyle(color: _selectedCategoriaId == null ? Color(0xFFF44336) : Colors.black87, fontWeight: FontWeight.bold),
                      ),
                      if (_selectedCategoriaId == null) Container(margin: EdgeInsets.only(top: 2), height: 3, width: 32, color: Color(0xFFF44336)),
                    ],
                  ),
                ),
                SizedBox(width: 24),
                  ...
                  (cat) => GestureDetector(
                    ...
                    child: Column(
                      children: [
                        Text(
                          cat.nome ?? '',
                          style: TextStyle(color: _selectedCategoriaId == cat.id ? Color(0xFFF44336) : Colors.black87, fontWeight: FontWeight.bold),
                        ),
                        if (_selectedCategoriaId == cat.id) Container(margin: EdgeInsets.only(top: 2), height: 3, width: 32, color: Color(0xFFF44336)),
                      ],
                    ),
                  ),
                ),
          ...
          Expanded(
            child: GridView.builder(
              ...
              gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                crossAxisCount: 3,
                childAspectRatio: 0.7, // Mantém proporção dos cards
                crossAxisSpacing: 12,
                mainAxisSpacing: 12,
              ),
                    ...
                    children: [
                      Container(
                        ...
                        child: Column(
                          children: [
                            AspectRatio(
                              aspectRatio: 1, // Imagem quadrada e proporcional
                              child: ClipRRect(
                                borderRadius: BorderRadius.circular(8),
                                child: produto.image_url != null && produto.image_url!.isNotEmpty
                                    ? Image.network(
                                        produto.image_url!,
                                        fit: BoxFit.cover,
                                        errorBuilder: (context, error, stackTrace) => Icon(Icons.fastfood, color: Color(0xFFF44336), size: 32),
                                      )
                                    : Container(
                                        color: Colors.grey[200],
                                        child: Icon(Icons.fastfood, color: Color(0xFFF44336), size: 32),
                                      ),
                              ),
                            ),
                            SizedBox(height: 8),
        ...
        children: [
            ...
            child: Card(
              ...
              child: Padding(
                ...
                child: itensCarrinho.isEmpty
                    ? Center(
                        child: Text('Nenhum produto selecionado', style: TextStyle(color: Colors.black54)),
                      )
                    : ListView.builder(
                              ...
                              children: [
                                Expanded(
                                  child: Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      Text(produto.descricao ?? '', style: TextStyle(fontWeight: FontWeight.bold)),
                                      Text('$qtd UN x R\$ ${produto.preco?.toStringAsFixed(2) ?? '0,00'}', style: TextStyle(color: Colors.black54, fontSize: 13)),
                                    ],
                                  ),
                                ),
                                Text('R\$ ${valorTotal.toStringAsFixed(2)}', style: TextStyle(fontWeight: FontWeight.bold)),
                                IconButton(
                                  icon: Icon(Icons.delete, color: Color(0xFFF44336), size: 20),
          ...
          Row(
            ...
            children: [
              ...
              Text('R\$ ${subtotal.toStringAsFixed(2)}', style: TextStyle(fontWeight: FontWeight.bold)),
            ],
          ),
          SizedBox(height: 4),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('Desconto na venda', style: TextStyle(color: Colors.black54)),
              Text('- R\$ ${desconto.toStringAsFixed(2)}', style: TextStyle(color: Colors.black54)),
            ],
          ),
          SizedBox(height: 4),
          Row(
            ...
            children: [
              ...
              Text('R\$ ${total.toStringAsFixed(2)}', style: TextStyle(fontWeight: FontWeight.bold)),
            ],
          ),
        ],
      ),
